...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/wangn9900/Tox-script/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/wangn9900/Tox-script/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/tox -N --no-check-certificate https://raw.githubusercontent.com/wangn9900/Tox-script/main/tox.sh
    if [[ $? != 0 ]]; then
        echo ""
