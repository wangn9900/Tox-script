...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/wangn9900/V2bX-TQP/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            last_version="tqp-latest"
        fi
        echo -e "检测到 Tox 最新版本：${last_version}，开始安装"
        wget --no-check-certificate -N --progress=bar -O /usr/local/tox/tox https://github.com/wangn9900/V2bX-TQP/releases/download/tqp-latest/V2bX-TQP-linux-${arch}
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/wangn9900/V2bX-TQP/releases/download/${last_version}/V2bX-TQP-linux-${arch}"
        echo -e "开始安装 Tox $1"
        wget --no-check-certificate -N --progress=bar -O /usr/local/tox/tox ${url}
        if [[ $? -ne 0 ]]; then
